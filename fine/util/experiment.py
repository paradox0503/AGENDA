...
from util.dataset_configs import *
    ...
    def __validate(self,func_a: float) -> None:
        ...
        with torch.no_grad():
            for db_batch, query_batch1,query_batch2 in zip(self.val_db_loader, self.val_query_loader1,self.val_query_loader2):
                ...
                if not self.encoder_only:
                    # print("????-------------------------------------------------------------------------!!")
                    agenda_db_recon = self.model.decode(db_embedding)#zijixied
                    agenda_recons_term = self.recons_weight * self.recons_reg(db_batch, agenda_db_recon)#计算重构误差
                else:
                    # print("!!!!-------------------------------------------------------------------------!!")
                    agenda_recons_term = torch.zeros(1).to(self.device)
                # t = min(db_batch.shape[0], agenda_db_recon.shape[0])
                # agenda_recons_term = self.recons_weight * self.recons_reg(db_batch[:t, :], agenda_db_recon[:t, :])
                agenda_orth_term = self.__orth_reg()
                agenda_regularization = agenda_recons_term + agenda_orth_term
                if self.__conf.getHP("mode")=="pretrain":
                    return_l2=mean(self.__l2(squeeze(db_orig), squeeze(db_batch)))*(0.0001+abs(self.model._AEBuilder__encoder.fucb))
                ...
                # trans_error = self.trans_loss(func_a,db_batch, query_batch1, db_embedding, query_embedding1)
                loss=agenda_regularization+trans_error   #+return_l2 #jiade
                # print(trans_error)
                trans_errors.append(trans_error.detach().item())
                agenda_recons.append(agenda_recons_term.detach().item())
                agenda_regulars.append(agenda_regularization.detach().item())
                agenda_losss.append(loss.detach().item())
        print("本轮trans_error = ",np.mean(trans_errors))
        print("本轮全部的loss = ",np.mean(agenda_losss))
        self.logger.info('v{:d} trans = {:.4f}'.format(self.epoch, np.mean(trans_errors)))#搞出trans error的值选择a
        self.logger.info('v{:d} recons = {:.4f}'.format(self.epoch, np.mean(agenda_recons)))
        self.logger.info('v{:d} regular = {:.4f}'.format(self.epoch, np.mean(agenda_regulars)))
        self.logger.info('v{:d} loss = {:.4f}'.format(self.epoch, np.mean(agenda_losss)))
