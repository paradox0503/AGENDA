...
int main() {
    uint64_t query_num = std::stoi(read_nth_line("/data/AGENDA/isax/index.txt", 1));//+
    const uint64_t k = std::stoi(read_nth_line("/data/AGENDA/isax/index.txt", 2)); //+
    const uint64_t index_construct = 1;
    const string data_name = read_nth_line("/data/AGENDA/isax/index.txt", 3);  //+
    static const std::string origin_input_directory = read_nth_line("/data/AGENDA/isax/index.txt", 4);//+
    static const std::string origin_query_directory = origin_input_directory;
    static const std::string embed_input_directory = read_nth_line("/data/AGENDA/isax/index.txt", 5);//+
    ...
    uint32_t source_value =std::stoi(read_nth_line("/data/AGENDA/isax/index.txt", 6));//+
    const uint32_t ts_length = source_value;//
    const uint64_t ts_num = std::stoi(read_nth_line("/data/AGENDA/isax/index.txt", 7));//+
    ...
    uint32_t ref_objs_size=std::stoi(read_nth_line("/data/AGENDA/isax/index.txt", 8));//+
    uint32_t approximate_leaf_size=std::stoi(read_nth_line("/data/AGENDA/isax/index.txt", 9));//+
    uint64_t ts_buffer_size_for_read=std::stoi(read_nth_line("/data/AGENDA/isax/index.txt", 10));//+
    uint64_t ts_buffer_size_per_ref_obj=std::stoi(read_nth_line("/data/AGENDA/isax/index.txt",11));//+


    if (ts_length == 256){
    isax::iSAXFactory<256, sax_length>::create(
    ...
    isax::iSAXFactory<128, sax_length>::create(
        ts_num, ref_objs_size, approximate_leaf_size, ts_buffer_size_for_read, ts_buffer_size_per_ref_obj, data_name, input_filename, output_directory,emb_input_filename
    );
    }else if(ts_length == 96){
    isax::iSAXFactory<96, sax_length>::create(
        ts_num, ref_objs_size, approximate_leaf_size, ts_buffer_size_for_read, ts_buffer_size_per_ref_obj, data_name, input_filename, output_directory,emb_input_filename
    );
    }
