...
| F5                         |                                                                             is a synthetic time-series dataset consisting of fixed-length sequences (256). Each sequence is generated in the frequency domain by selecting a small number of dominant frequency components and then transformed into the temporal domain via inverse discrete Fourier transform (IDFT).                                                                             |
...
| :--------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
| PAA                      |                                                             Piecewise Aggregate Approximation, a classic dimensionality reduction technique that reduces the volume of time series data by dividing the sequence into equal frames and calculating the mean value for each frame.                                              |
| SPARTAN                         |                        is a data-adaptive symbolic time-series approximation method that allocates encoding budget to the most informative parts to form compact symbolic representations.                       |
